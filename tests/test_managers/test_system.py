...
async def test_check_outdated_parses_output():
    ...
    with patch(
        "pkg_upgrade.managers.system.run_command", new=AsyncMock(return_value=(0, output, ""))
    ):
...
async def test_check_outdated_no_updates():
    with patch(
        "pkg_upgrade.managers.system.run_command",
...
async def test_upgrade_success():
    ...
    with patch(
        "pkg_upgrade.managers.system.run_command", new=AsyncMock(return_value=(0, "Done", ""))
    ):
