...
async def test_check_outdated_parses_output():
    ...
    with patch(
        "pkg_upgrade.managers.gem.run_command", new=AsyncMock(return_value=(0, gem_output, ""))
    ):
...
async def test_check_outdated_empty():
    with patch("pkg_upgrade.managers.gem.run_command", new=AsyncMock(return_value=(0, "", ""))):
...
async def test_upgrade_success():
    ...
    with patch(
        "pkg_upgrade.managers.gem.run_command", new=AsyncMock(return_value=(0, "Installed", ""))
    ):
